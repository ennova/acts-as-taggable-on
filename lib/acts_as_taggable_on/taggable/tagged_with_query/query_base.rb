  ...
  class QueryBase
    ...
    def tag_match_type(tag)
      ...
      if options[:wild].present?
        tag_arel_table[:name].matches("%#{escaped_tag(tag)}%", ActsAsTaggableOn::Utils.escape_replacement)
      else
        tag_arel_table[:name].matches(escaped_tag(tag), ActsAsTaggableOn::Utils.escape_replacement)
      end
    end
    ...
    def tags_match_type
      ...
      if options[:wild].present?
        matches_attribute.matches_any(tag_list.map{|tag| "%#{escaped_tag(tag)}%"}, ActsAsTaggableOn::Utils.escape_replacement)
      else
        matches_attribute.matches_any(tag_list.map{|tag| "#{escaped_tag(tag)}"}, ActsAsTaggableOn::Utils.escape_replacement)
      end
    end
